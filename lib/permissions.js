...
function setPermission(userId, guildId, permissionType, remove = false) {
    ...
    if (remove) {
        if (userIndex !== -1) {
            ...
            changed = true;
            console.log(`Removed ${userId} from ${normalizedType} permissions in guild ${guildId}`);
        } else {
            console.log(`User ${userId} was not in ${normalizedType} permissions for guild ${guildId}`);
        }
    } else {
        if (userIndex === -1) {
            permissionArray.push(userId);
            changed = true;
            console.log(`Added ${userId} to ${normalizedType} permissions in guild ${guildId}`);
        } else {
            console.log(`User ${userId} already had ${normalizedType} permissions in guild ${guildId}`);
        }
    }
