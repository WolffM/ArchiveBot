...
async function initializeDatabaseIfNeeded(guildId) {
    ...
    if (fs.existsSync(dbPath)) {
        log.info('initializeDatabase', { guildId, status: 'already exists' });
        return;
    }
    ...
    try {
        ...
        const files = fs.readdirSync(channelPath)
            ...
            .filter(file => {
                ...
                if (isProcessed) {
                    log.debug('processNewArchiveFiles', {
        ...
        if (files.length === 0) {
            log.info('processNewArchiveFiles', { guildId, result: 'No new files' });
            return;
        }

        log.info('processNewArchiveFiles', { guildId, fileCount: files.length });
        ...
        log.error('processNewArchiveFiles', error, { guildId });
        throw error;
    }
}
