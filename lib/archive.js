...
async function updateReactionData(messages) {
    ...
    for (let i = 0; i < messages.length; i += 100) {
        ...
        if (!channel) {
            log.warn('updateReactionData', {
                batchStart: i,
                reason: 'null channel reference, skipping batch'
            });
            continue;
        }
        try {
...
async function archiveChannel(channel, options = { saveMessages: true, saveAttachments: true }) {
    ...
    // Update reactions (non-fatal — save the archive even if reactions fail)
    try {
        await updateReactionData(allMessages);
    } catch (err) {
        log.warn('archiveChannel', {
            channelId: channel.id,
            channelName: channel.name,
            phase: 'updateReactionData',
            error: err.message
        });
    }
...
async function fetchMessageBatch(channel, lastArchiveTime) {
    ...
    while (keepFetching) {
        ...
        try {
            ...
            // discord.js can crash constructing Poll/other objects from API data
            // (e.g., expired polls with null fields). Skip this batch and continue.
            log.error('fetchMessageBatch', fetchErr, {
                channelId: channel.id,
                channelName: channel.name,
                batch: batchCount,
                lastId,
                reason: 'fetch failed, skipping batch'
            });
            // If we have a lastId, try to continue past the problematic batch
            if (lastId) {
                batchCount++;
                await delay(1000);
                continue;
...
async function ingestOrphanedFiles(guildId) {
    const guildDir = path.join(__dirname, '..', 'Output', guildId);
    if (!fs.existsSync(guildDir)) return;

    const dbPath = path.join(guildDir, 'archive.db');
    let db;
    try {
        db = await open({ filename: dbPath, driver: sqlite3.Database });
        ...
        for (const entry of entries) {
            ...
            const channelPath = path.join(guildDir, entry.name);
            const { channelId } = parseChannelFolder(channelPath);
            ...
            const result = await processNewArchiveFiles(db, guildId, channelPath);
        ...
        await db.close();
        if (ingested > 0) {
            log.info('ingestOrphanedFiles', { guildId, ingested });
        }
    } catch (err) {
        if (db) await db.close().catch(() => {});
        log.error('ingestOrphanedFiles', err, { guildId });
    }
}
...
async function fetchForumThreads(guild) {
    const forumChannels = guild.channels.cache.filter(ch => ch.type === 15);
    const results = [];
    ...
    for (const [, forum] of forumChannels) {
        ...
        try {
            ...
            log.error('fetchForumThreads', err, { forumId: forum.id, forumName: forum.name, phase: 'fetchActive' });
        }
        ...
        try {
            ...
            log.error('fetchForumThreads', err, { forumId: forum.id, forumName: forum.name, phase: 'fetchArchived' });
        }

        if (threads.length > 0) {
            results.push({ forum, threads });
        }
    }
...
function classifyArchiveError(err) {
    const code = err.code;
    if (code === 50001) return 'missing access';
    if (code === 50013) return 'missing permissions';
    if (code === 10003) return 'unknown channel';
    if (code === 10008) return 'unknown message';
    if (code === 429 || err.httpStatus === 429) return 'rate limited';
    return 'error';
}
...
async function archiveServer(guild, statusChannel, options = { saveMessages: true, saveAttachments: true }) {
    ...
    // Sweep: ingest any orphaned JSON files from previous interrupted runs
    await ingestOrphanedFiles(guild.id);

    const textChannels = guild.channels.cache.filter(ch => ch.type === 0);
    const forumData = await fetchForumThreads(guild);
    const forumPostCount = forumData.reduce((sum, f) => sum + f.threads.length, 0);

    const total = textChannels.size + forumPostCount;
    let processed = 0;
    let archived = 0;
    let skipped = 0;
    let errors = 0;

    const parts = [`${textChannels.size} text channel(s)`];
    if (forumPostCount > 0) {
        parts.push(`${forumPostCount} forum post(s) across ${forumData.length} forum(s)`);
    }
    await statusChannel.send(`Starting server archive — ${parts.join(', ')} to process.`);
    ...
    for (const [, ch] of textChannels) {
        try {
            ...
            processed++;
            errors++;
            const reason = classifyArchiveError(err);
            log.error('archiveServer', err, { channelId: ch.id, channelName: ch.name, reason });
            await statusChannel.send(`[${processed}/${total}] #${ch.name} — ${reason}: ${err.message}`);
        }
    }

    for (const { forum, threads } of forumData) {
        for (const thread of threads) {
            try {
                const result = await archiveChannel(thread, options);
                processed++;
                if (result) {
                    archived++;
                    await statusChannel.send(`[${processed}/${total}] ${forum.name} > ${thread.name} — archived`);
                } else {
                    skipped++;
                }
            } catch (err) {
                ...
                errors++;
                const reason = classifyArchiveError(err);
                log.error('archiveServer', err, { forumId: forum.id, threadId: thread.id, threadName: thread.name, reason });
                await statusChannel.send(`[${processed}/${total}] ${forum.name} > ${thread.name} — ${reason}: ${err.message}`);
            }
        }
    }
