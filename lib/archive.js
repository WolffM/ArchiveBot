    ...
    for (const { forum, threads } of forumData) {
        for (const thread of threads) {
            try {
                ...
                processed++;
                errors++;
                log.error('archiveServer', err, { forumId: forum.id, threadId: thread.id, threadName: thread.name });
                await statusChannel.send(`[${processed}/${total}] ${forum.name} > ${thread.name} — error: ${err.message}`);
            }
        }
    }
