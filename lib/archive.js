...
async function archiveServer(guild, statusChannel) {
    ...
    for (const [, ch] of textChannels) {
        try {
            const result = await archiveChannel(ch, { saveMessages: true, saveAttachments: true });
            if (result) archived++;
        } catch (err) {
            errors++;
    ...
    await statusChannel.send(
        `Archive complete: ${archived} channel(s) archived` +
