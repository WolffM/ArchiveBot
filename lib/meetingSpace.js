 ...
 *
 * Permissions are set ONCE on the category and both channels inherit, so a
 * space's visibility is one object's business rather than three.
 *
 * It does NOT make teardown a single call. This paragraph used to claim that
 * deleting the category takes its children with it, and that the design
 * therefore could not "half-succeed and leave an orphaned voice channel nobody
 * can see" — which is precisely what it then did. Discord leaves the children
 * alive and merely uncategorised, and because their space record was already
 * marked inactive, no later sweep ever looked at them again. The teardown in
 * meetingSpaceReaper.js deletes the children FIRST for that reason; the
 * permission argument above is the only thing the category buys.
 *
 ...
 *
 * `title` is the ONE thing a caller can have shown, and it is not an exception
 * to that rule. It goes in a message body INSIDE the space, which only the one
 * guest holding the space role and the owner can read — it is never a Discord
 * object name, so it never reaches autocomplete or the member list. `label`
 * stays operator-only.
 *
 * ## What a space says
 *
 * An intro post on provision, and an `@everyone` five minutes before the start,
 * both in the space's own `chat`. They exist because a space carries no time of
 * its own: Discord's scheduled events would show one, but an event is a
 * guild-wide object and these meetings are one-guest-each, so the only thing
 * that could display the time is the one thing that cannot stay private.
 *
 * Both are driven by the optional `startsAt` a caller may pass. Neither needs
 * reaping: the intro lives in a channel teardown deletes, and the reminder's
 * schedule lives on the space RECORD, so a revoked or swept space takes its
 * pending reminder with it. See meetingSpaceAnnounce.js.
 *
...
const { postIntro, REMINDER_LEAD_MS } = require('./meetingSpaceAnnounce');
 ...
 *
 * Raised from 31 days, which silently capped the wrong thing. A caller's TTL
 * has to cover the wait UNTIL the meeting plus whatever grace it wants after —
 * contact-api books up to 30 days out and keeps a space 8 days past the end, so
 * its furthest booking asks for ~38 days and was refused `ttl_too_long`. The
 * booking still succeeded (a meeting link never fails a booking) and the guest
 * got no space at all, which is the worst of both.
 *
 * 45 days is that worst case with room to move, and still far short of anything
 * that could accumulate against the channel ceiling: the sweep runs daily, so
 * the ceiling is bounded by the booking RATE, not by this number.
 */
const MAX_TTL_MS = 45 * 24 * 60 * 60 * 1000;
...
function validateProvisionPayload(payload) {
    ...
    // Optional, and optional all the way down: a caller with no time to give
    // gets a space with no intro timestamp and no reminder rather than an
    // error. Nothing here knows what a booking is — see the module comment.
    let startsAt = null;
    if (payload.startsAt !== undefined && payload.startsAt !== null) {
        if (typeof payload.startsAt !== 'string' || !Number.isFinite(Date.parse(payload.startsAt))) {
            return fail('invalid_starts_at');
        }
        startsAt = new Date(Date.parse(payload.startsAt)).toISOString();
    }

    // Shown INSIDE the space, unlike `label`. That is safe for the same reason
    // the channels are: the only people who can read it are the one guest
    // holding the space role and the owner. It is still capped and trimmed,
    // because it lands in a message body.
    let title = null;
...
async function handleSpaceProvision(payload, discordClient) {
    ...
    try {
        ...
        // REFRESH THE INVITE CACHE so this code is in the "before" snapshot.
        //
        // Without this the binding cannot work for any first join, and not as a
        // race — structurally. handleMemberAdd attributes a join by diffing the
        // cached invite list against the live one, and a maxUses:1 invite is
        // consumed (and deleted by Discord) the moment it is used. If the cache
        // was last taken at boot, the new code is in NEITHER snapshot, so
        // nothing appears to have vanished and every join logs
        // `join_unattributed`. The guest lands in the guild with no role and no
        // private space, and nothing errors.
        //
        // Awaited, not fired and forgotten: the invite URL is handed back in
        // this response and the guest can act on it immediately.
        await require('./meetingSpaceReaper').cacheGuildInvites(guild);
    } catch (err) {
    ...
    const record = {
        ...
        // Named, not channelIds[0]. The announce path writes here and an array
        // index is not a contract — reordering the two creates above would have
        // sent the intro into the voice channel.
        chatChannelId: chatChannelId,
        title: input.title,
        startsAt: input.startsAt,
        // Armed only when there is a start to count back from. Null means "no
        // reminder", which findDueReminders reads as nothing to do — there is no
        // separate "reminders enabled" flag to fall out of step with it.
        remindAt: input.startsAt
            ? new Date(Date.parse(input.startsAt) - REMINDER_LEAD_MS).toISOString()
            : null,
        remindedAt: null,
    ...
    // AFTER the store write, and deliberately not inside the try above. The
    // space exists and works at this point; a greeting that fails must not
    // trigger the rollback that throws the whole thing away. postIntro never
    // throws and reports its own failure.
    await postIntro(guild, record);
...
/**
 * The repair pass as an HTTP route, so orphans left by an older teardown can be
 * collected without a redeploy or a shell on the host.
 */
async function handleSpaceRepair(payload, discordClient) {
    const envelope = validateEnvelope(payload);
    if (envelope) return { status: 400, body: { success: false, error: envelope } };

    const { repairOrphanedSpaces } = require('./meetingSpaceReaper');
    const summary = await repairOrphanedSpaces(discordClient);
    return { status: 200, body: { success: true, ...summary } };
}
