 ...
 * ride a Discord-native recurrence rule, so the live scheduledStartTime is the
...
async function reconcileWithScheduledEvent(guild, item, { force = false } = {}) {
    ...
    const liveStart = scheduledEvent.scheduledStartTime;
...
async function handleScheduledEventUpdate(_oldEvent, newEvent) {
    ...
    if (newStartTime && reminderItem && Number.isFinite(reminderItem.remindBeforeMs)) {
        const newReminderTime = new Date(
            newEvent.scheduledStartTime.getTime() - reminderItem.remindBeforeMs
        ).toISOString();
