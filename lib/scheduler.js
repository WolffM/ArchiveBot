    ...
    try {
        ...
        if (removed.scheduledEventId) {
            try {
                ...
                // Event may already be deleted or not exist
                if (eventError.code !== 10070) { // Unknown Guild Scheduled Event
        ...
        if (removed.type === 'event_reminder' && removed.scheduledEventId) {
            // Find and remove the parent event
        ...
        let replyMessage = `Removed ${typeLabel} #${id}`;
        if (removed.message) {
            replyMessage += `: "${removed.message}"`;
        } else if (removed.eventName) {
            replyMessage += `: "${removed.eventName}"`;
        }
        if (removed.scheduledEventId && (removed.type === 'event' || removed.type === 'event_reminder')) {
            replyMessage += '\nDiscord Scheduled Event has been deleted.';
        }
...
async function handleEventCommand(interaction) {
    ...
    try {
        ...
        if (remindBeforeMs) {
            ...
            // Calculate recurring pattern to match event
            let reminderRecurring = null;
            if (recurring) {
                const recurringMap = {
                    'daily': '1d',
                    'weekly': '1w',
                ...
                reminderRecurring = recurringMap[recurring];
            }
