...
function parseRelativeTime(input) {
    ...
    // Match patterns like: 3h, 30m, 1d, 2w, 1h30m
    const match = trimmed.match(/^(\d+)\s*(m|min|mins|minutes?|h|hr|hrs|hours?|d|days?|w|wk|wks|weeks?)$/);
    if (match) {
...
/**
 * Parse recurring pattern: every1d, every1w, every1m, every1y
...
async function checkAllItems() {
    if (!discordClient) {
        console.warn('Discord client not initialized for scheduler');
        return;
    }
    ...
    for (const guild of discordClient.guilds.cache.values()) {
        try {
            ...
            console.error(`Error checking scheduled items for guild ${guild.id}:`, error);
        }
    }
}
    ...
    for (const item of data.items) {
        ...
        if (triggerTime <= now) {
            try {
                ...
                console.error(`Error firing item ${item.id}:`, error);

                if (error.code === 10003) { // Unknown Channel
                    item.active = false;
                    modified = true;
                    console.log(`Deactivated item ${item.id} - channel no longer exists`);
                }
            }
        }
    }
...
async function fireItem(guild, item) {
    ...
    console.log(`Fired ${item.type} #${item.id} in channel ${item.channelId}`);
}
...
async function handleAddCommand(interaction) {
    ...
    try {
        ...
        const triggerDate = parseDateTime(at);
        if (!triggerDate) {
            await interaction.editReply(
                'Invalid datetime format. Use:\n' +
                '• `2026-01-20 10:00` - specific date and time\n' +
                '• `10:00` - today/tomorrow at that time\n' +
                '• `tomorrow 14:30` - tomorrow at that time'
            );
            return;
        }
        ...
        console.error('Error adding scheduled item:', error);
...
async function handleShowCommand(interaction) {
    ...
    try {
        ...
        console.error('Error showing scheduled items:', error);
        await interaction.editReply(`Error: ${error.message}`);
    }
}
...
async function handleMessageReminder(message) {
    ...
    if (!triggerDate) {
        await message.reply({
            content: 'Invalid time format. Try: `remind me in 3h`, `remind me in 30m`, `remind me in 1d`',
    ...
    try {
        ...
        await message.reply({
            content: `Got it! I'll remind you about that message in ${formatRelativeTime(timeUntil)}.`,
            allowedMentions: { repliedUser: false }
        });

        console.log(`Created personal reminder #${newItem.id} for user ${message.author.id}`);
        return true;
