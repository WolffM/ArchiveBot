...
async function fireItem(guild, item) {
    ...
    // Execute attached action (e.g. pickleball automation) — fire-and-forget
    if (item.action) {
        const pickleball = require('./pickleball');
        pickleball.executeAction(item.action, channel).catch(err => {
