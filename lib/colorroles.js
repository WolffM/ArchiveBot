...
const log = createLogger('colorroles');
...
async function loadOrInferColorRoles(guild) {
    try {
        ...
        log.debug('loadOrInferColorRoles', { guildId: guild.id, filePath });
        
        // Try to load from file first
        if (fs.existsSync(filePath)) {
            log.info('loadOrInferColorRoles', { guildId: guild.id, source: 'file' });
            return loadColorRoles(guild.id);
        }
        
        // If file doesn't exist, infer from guild roles
        log.info('loadOrInferColorRoles', { guildId: guild.id, source: 'inferred from guild' });
        return await inferColorRolesFromGuild(guild);
    } catch (error) {
        log.error('loadOrInferColorRoles', error, { guildId: guild.id });
...
async function createColorRole(guild, hexColor) {
    ...
    try {
        log.info('createColorRole', { guildId: guild.id, roleName, hexColor });
        ...
        if (colorRoleIds.length > 0) {
            ...
            if (existingColorRoles.size > 0) {
                ...
                log.debug('createColorRole', { phase: 'position', position });
            }
        }
        ...
        log.success('createColorRole', { guildId: guild.id, roleId: newRole.id, roleName });
        ...
        log.info('createColorRole', { phase: 'saved', roleName, hexColor });
        ...
        if (!savedRole) {
            log.warn('createColorRole', { roleId: newRole.id, reason: 'Role may not have been saved' });
        }
        ...
        log.error('createColorRole', error, { guildId: guild.id, hexColor });
...
async function removeExistingColorRoles(member, colorRoles) {
    try {
        ...
        if (rolesToRemove.length > 0) {
            ...
            log.success('removeExistingColorRoles', {
                userId: member.user.id,
                rolesRemoved: rolesToRemove.length
            });
        }
    } catch (error) {
        log.error('removeExistingColorRoles', error, { userId: member.user.id });
...
async function applyColorRole(interaction, role, colorRoles) {
    try {
        ...
        log.error('applyColorRole', error, {
            userId: interaction.user.id,
            guildId: interaction.guild.id
        });
...
async function handleColorShowCommand(interaction) {
    ...
    try {
        log.info('handleColorShowCommand', { guildId: interaction.guild.id });
        ...
        try {
            ...
            log.error('handleColorShowCommand', error, { guildId: interaction.guild.id });
...
async function handleColorCommand(interaction) {
    ...
    try {
        log.info('handleColorCommand', { guildId: interaction.guild.id });
        ...
        log.debug('handleColorCommand', { identifier });
        ensureGuildDirectory(interaction.guild.id);
        ...
        try {
            ...
            log.error('handleColorCommand', error, { guildId: interaction.guild.id, phase: 'load roles' });
            if (error.message.includes('No colored roles found')) {
                await interaction.editReply('There are no colored roles in this server. Please ask a server admin to create some roles with colors first.');
            } else {
        ...
        if (numberMatch) {
            ...
            if (index >= 0 && index < sortedRoles.length) {
                log.debug('handleColorCommand', { index: index + 1, roleName: sortedRoles[index].name });
        ...
        if (!role.id || !interaction.guild.roles.cache.get(role.id)) {
            try {
                log.info('handleColorCommand', { phase: !role.id ? 'creating' : 'recreating', hexColor: role.hexColor });
        ...
        log.error('handleColorCommand', error, { guildId: interaction.guild.id });
