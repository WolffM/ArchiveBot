...
async function executeAction(action, channel) {
    if (!SCRAPE_SERVICE_KEY) {
        log.warn('executeAction', { reason: 'SCRAPE_SERVICE_KEY not configured, skipping action', action });
        return;
    }
...
async function runSignupAction(channel) {
    // Fire-and-walk-away: the scraper pushes the outcome to our webhook
    // (see lib/webhookServer.js) when the signup completes — success,
    // failure, sold-out, waitlist. We do NOT poll. Polling against a
    // signup that can legitimately take many minutes (multi-candidate
    // detail-page verification on busy nights) produced silent "timed
    // out after 90s" failures while the scraper kept running and signed
    // up successfully. The webhook is the source of truth.
    try {
        const { data: resp } = await api.post('/api/v1/pickleball/signup', SIGNUP_PARAMS);

        if (!resp.success) {
            const errMsg = resp.error?.message || 'Unknown error';
            log.error('runSignupAction', errMsg, { reason: 'API returned failure' });
            await channel.send(`Failed to start pickleball signup: ${errMsg}`);
            return;
        }

        await channel.send(
            'Pickleball signup started — outcome will follow here when done.'
        );
    } catch (error) {
        const detail = extractApiErrorMessage(error);
        log.error('runSignupAction', error, { status: error.response?.status, detail });
        await channel.send(`Pickleball signup error: ${detail}`).catch(() => {});
    }
}
...
async function runFindAction(channel) {
    ...
    try {
        ...
        if (!resp.success) {
            ...
            log.error('runFindAction', errMsg, { reason: 'API returned failure' });
