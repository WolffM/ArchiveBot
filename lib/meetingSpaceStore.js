 ...
 *   title / startsAt
 *                   what the space is for and when it begins, both optional —
 *                   a caller that supplies neither gets a space with no intro
 *                   and no reminder, which is the generic case
 *   chatChannelId   the text channel, named rather than taken from
 *                   channelIds[0]; the announce path writes to it and an
 *                   index is not a contract
 *   remindAt / remindedAt / reminderSkipped
 *                   when the pre-meeting ping is due, and how it was retired.
 *                   Kept HERE rather than in the scheduler's announce queue so
 *                   a torn-down space cannot leave a reminder pointing at a
 *                   channel that no longer exists — see meetingSpaceAnnounce.
...
/**
 * Where the per-guild files live.
 *
 * Overridable because `repairOrphanedSpaces` and `sweepExpiredSpaces` walk
 * EVERY guild directory under this root — that is correct for the bot, which
 * has one root and must not miss a guild, and it made the tests share one. A
 * test seeding a single guild saw the leftovers of every test file that ran
 * before it, so `skippedActive` counted four spaces instead of the one it had
 * created, and which suite failed depended on the order jest happened to pick.
 *
 * Read at require time on purpose: these are module-level paths, and a value
 * that could change under a running bot would be worse than one that cannot.
 */
const SPACES_DIR = process.env.ARCHIVEBOT_OUTPUT_DIR || path.join(__dirname, '..', 'Output');
...
/**
 * Reminders that have come due and have not been retired.
 *
 * `stuck` spaces are excluded for the same reason the sweep excludes them: they
 * are known-broken and retrying anything against them only makes noise.
 */
function findDueReminders(guildId, now = Date.now()) {
    return load(guildId).spaces.filter(
        (s) =>
            s.active &&
            !s.stuck &&
            s.remindAt &&
            !s.remindedAt &&
            Date.parse(s.remindAt) <= now
    );
}

/**
 * Retire a reminder so it fires at most once.
 *
 * Stamped whether it was sent or abandoned — `reminderSkipped` says which. An
 * unstamped record is one the next tick should try again, so every terminal
 * outcome has to come through here or the 60-second tick retries it forever.
 */
function markReminded(guildId, spaceId, extra = {}) {
    const data = load(guildId);
    const space = data.spaces.find((s) => s.spaceId === spaceId);
    if (!space) return null;
    space.remindedAt = new Date().toISOString();
