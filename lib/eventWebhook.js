...
async function handleEventsChannels(payload, discordClient) {
    ...
    // would omit the single most likely pick. Only ACTIVE threads are in the
    // cache, which is the behaviour we want — an archived thread should not be
    // offered. Voice and category channels stay out: a category has no send at
    // all, and a voice channel's text tab is not where anyone looks for this.
