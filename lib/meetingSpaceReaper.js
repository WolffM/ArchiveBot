 ...
 * ## The sweep
 *
 * Deletes an expired space's child channels, THEN its category, THEN its role,
 * and kicks the member who joined through it. The order is not cosmetic —
 * Discord does not cascade a category delete to its children (see
 * `teardownSpaceChannels` below); this docstring claimed for a while that it
 * did, which is the assumption that left loose `chat` and `Meeting Room`
 * channels behind. This is NOT tidiness:
...
async function repairOrphanedSpaces(discordClient) {
    const summary = { checked: 0, skippedActive: 0, channels: 0, categories: 0, roles: 0, failed: 0 };
    ...
    for (const guildId of store.listGuildIds()) {
        ...
        for (const space of store.load(guildId).spaces) {
            // ONLY spaces already torn down. An ACTIVE space belongs to a
            // booking that has not happened yet, and repair deleting it would
            // destroy a live meeting's channels and invalidate the invite its
            // guest was already sent — which is exactly what happened on
            // 2026-09-07 when this loop walked every record. Expiry is the
            // sweep's job and it has a clock; repair exists solely to collect
            // what a PAST teardown left behind.
            if (space.active) {
