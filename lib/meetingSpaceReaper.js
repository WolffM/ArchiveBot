 ...
 * the channel and role an operator must delete, and then left alone. A space
 * that fails the job every minute forever is not an alarm, it is background
...
/**
 * How many failed sweeps before a space is parked as stuck.
 *
 * Three rides out a transient Discord outage or a 5xx without parking a space
 * that would have healed by itself, and still goes quiet within minutes rather
 * than failing the cron every 60 seconds until somebody notices.
 */
...
async function recoverCategoryAccess(guild, space, reason) {
    ...
    try {
        ...
        if (category) await category.delete(reason);
